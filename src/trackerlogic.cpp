    ...
    if (no_nfc_count < READER_COUNT) {
        if (door_closed_count == DOOR_CLOSED_REPORT_THRESHOLD) {
            ...
            if (alert_redundant_nfc_array.size() > 0) {
                ...
                for (int i = 0; i < arr_size; i++) {
                    String reader_id = String(arr[i]+1);
                    content += reader_id;
                    if (i < arr_size - 1) { content += ", "; }
                }
                alert_json["content"] = content;
...
void updateReaderStatus(NfcAdapter &nfc, int reader_num) {
    String* reader_result = nfc_read(nfc);
    ...
    if (supposed_instance_idx != -1) { // supposed to have nfc/server being detected
        // Serial.println(reader_result[1]);
        if (reader_result[1] == "NO_NFC") { // !nfc.tagPresent(...)
            no_nfc_count++;
            if (count_missing[reader_idx] >= ALERT_THRESHOLD) {
                if (rack_json["data"][supposed_instance_idx]["status"] == PLAN_DEMMISSION) {
                    Serial.println("Reader " + String(reader_num) + ": ALERT(4) - decommissioned");
                    alert_decommission_instance_array.push_back(supposed_instance_idx);
                } else {
                    Serial.println("Reader " + String(reader_num) + ": ALERT(3) - missing nfc tag");
                    alert_missing_instance_array.push_back(supposed_instance_idx);
                }
            } else {
                Serial.println("Reader " + String(reader_num) + ": detecting...");
            ...
            if (reader_result[1].length() <= 5) {
                count_incorrect[reader_idx] = 0;
                if (count_empty[reader_idx] >= ALERT_THRESHOLD) {
                    Serial.println("Reader " + String(reader_num) + ": INFO(1) - added/wrote nfc content");
                    alert_empty_nfc_array.push_back(supposed_instance_idx);
                    nfc_write(nfc, reader_num, instance_id);
                    postServerAssetTag(instance_id, model_id, name, description, reader_result[0]);
                    if (door_closed_count < DOOR_CLOSED_REPORT_THRESHOLD) {
                        door_closed_count == DOOR_CLOSED_REPORT_THRESHOLD;
                    }
                } else {
                    Serial.println("Reader " + String(reader_num) + ": reading(empty)...");
                    count_empty[reader_idx]++;
                }
            } else if (reader_result[1].indexOf(instance_id) == -1) { // instance/server id does not exist(match) in nfc sticker
                count_empty[reader_idx] = 0;
                if (count_incorrect[reader_idx] >= ALERT_THRESHOLD) {
                    Serial.println("Reader " + String(reader_num) + ": ALERT(2) - unmatched nfc content");
                    alert_incorrect_instance_array.push_back(supposed_instance_idx);
                    // nfc_write(nfc, reader_num, instance_id); /////////// temperary
                    // postServerAssetTag(instance_id, reader_result[0]);  /////////// temperary
                    if (door_closed_count < DOOR_CLOSED_REPORT_THRESHOLD) {
                        door_closed_count == DOOR_CLOSED_REPORT_THRESHOLD;
                    }                
                } else {
                    Serial.println("Reader " + String(reader_num) + ": reading(incorrect)...");
                    count_incorrect[reader_idx]++;
                }
            } else {
                Serial.println("Reader " + String(reader_num) + ": INFO(5) - normal (correct)");
                count_empty[reader_idx] = 0;
                count_incorrect[reader_idx] = 0;
            }
        }
    } else { // not supposed to have nfc/server being detected
        if (reader_result[1] != "NO_NFC") {
            if (count_redundant[reader_idx] >= ALERT_THRESHOLD) {
                Serial.println("Reader " + String(reader_num) + ": ALERT(6) - unknown nfc tag detected");
                alert_redundant_nfc_array.push_back(reader_idx);
            } else {
                    Serial.println("Reader " + String(reader_num) + ": detecting (unknown)...");
                count_redundant[reader_idx]++;
            }
        } else {
            Serial.println("Reader " + String(reader_num) + ": null (not supposed to have nfc)");
            no_nfc_count++;
